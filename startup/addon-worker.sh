...
# Track whether anything changed this drain. Both installs and updates need a
# web-app restart at the end: an install adds a route (registered only at
# startup), and an update replaces a controller's .pm, which Mojolicious also
# loads only at startup -- so without the restart an updated addon keeps running
# its OLD code and typically breaks.
did_change=0
...
# Restart the web app once, after the whole queue drains, if anything installed
# or updated -- so new routes register AND updated controller/template code is
# actually loaded (Mojolicious reads controllers only at startup). This worker
# runs in its own transient systemd unit (systemd-run --collect), so it survives
# the restart and still exits cleanly.
[ "$did_change" = 1 ] && /usr/bin/systemctl restart easynas.service
